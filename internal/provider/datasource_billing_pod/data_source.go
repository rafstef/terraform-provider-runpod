...
func NewBillingPodDataSource() datasource.DataSource {
	return &BillingPodDataSource{}
}

type BillingPodDataSource struct{}

func (d *BillingPodDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = "runpod_billing_pod"
}

func (d *BillingPodDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = BillingPodDataSourceSchema(ctx)
}

func (d *BillingPodDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var config BillingPodModel
	...
	if !config.GpuTypeId.IsNull() {
		params["gpuTypeId"] = config.GpuTypeId.ValueString()
	}
	if !config.PodId.IsNull() {
		params["podId"] = config.PodId.ValueString()
	}
		...
		for i, record := range records {
			if recordMap, ok := record.(map[string]interface{}); ok {
				models[i] = BillingRecordModel{
					...
					GpuTypeId: func() types.String {
						if val, ok := recordMap["gpuTypeId"].(string); ok {
							return types.StringValue(val)
						}
						return types.StringNull()
					}(),
					PodId: func() types.String {
						if val, ok := recordMap["podId"].(string); ok {
							return types.StringValue(val)
						}
						return types.StringNull()
					}(),

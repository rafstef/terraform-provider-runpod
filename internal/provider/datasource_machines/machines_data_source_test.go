...
func TestMachinesRead_BlockedBySliceSchemaMismatch(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		_, _ = w.Write([]byte(`{"data":{"machines":[{"id":"m-123","name":"node-a","location":"US-CA","listed":true,"gpuType":"NVIDIA RTX 4090","gpuTotal":8,"secureCloud":true,"dataCenterId":"US-CA-1"}]}}`))
	}))
	defer srv.Close()

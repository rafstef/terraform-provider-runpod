...
func TestEndpointCreate_AllScalars(t *testing.T) {
	...
	for _, k := range []string{"computeType", "gpuCount", "vcpuCount", "workersMin", "workersMax", "idleTimeout", "scalerType", "scalerValue", "executionTimeoutMs", "flashboot", "minCudaVersion", "networkVolumeId"} {
		if _, ok := body[k]; !ok {
